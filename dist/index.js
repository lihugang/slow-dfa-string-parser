...
var DFA_map = (function () {
        ...
        for (var i = 0, len = descriptor.length; i < len; ++i) {
            ...
            for (var j = 0, jlen = currentStateArray.length; j < jlen; ++j) {
                ...
                if (!map_exist(nextState)) {
                    ...
                    nextState = map_get(nextState);
                this.states[currentState] = this.states[currentState] || {
                    '_do': {},
                    '_no_match_go_state': {}
                };
                var router = void 0;
                if (!(descriptor[i].match instanceof Array))
                    throw new TypeError("Invalid descriptor[".concat(i, "].match"));
                for (var k = 0, klen = descriptor[i].match.length; k < klen; ++k) {
                    ...
                    var end = router[1];
                    if (begin > end)
                        throw new RangeError('descriptor[${i}].match[${k}][0] must not be greater than descriptor[${i}].match[${k}][1]');
                    for (var l = begin; l <= end; l++) {
                        ...
                        if (descriptor[i]["do"]) {
                            if (descriptor[i]["do"] instanceof Function)
                                this.states[currentState]['_do'][l] = descriptor[i]["do"];
                            else
    ...
    DFA_map.prototype.next = function () {
        ...
        var ch = this.inputString.charCodeAt(this.readPosition);
        var nextState = this.states[currentState][ch];
        if (!nextState) {
        ...
        ;
        if (this.states[currentState]['_do'][ch])
            (_a = this.states[currentState]['_do'])[ch].apply(_a, __spreadArray([this.inputString[this.readPosition], this.statesMappingReverse[currentState], this.statesMappingReverse[nextState]], args, false));
        this.readPosition++;
